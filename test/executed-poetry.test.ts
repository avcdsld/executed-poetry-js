...
const text = (rel: string) => readFileSync(p(rel), "utf8");
const bytesOf = (rel: string): Hex => ("0x" + readFileSync(p(rel)).toString("hex")) as Hex;

type Manifest = { slug: string; title: string; poem: string }[];
const manifest: Manifest = JSON.parse(text("out/manifest.json"));

type Vector = {
  id: number; slug: string; title: string; env: string;
  n: number; micros: number; unix: number;
  pub: string; sig: string;
};
const vectors: Vector[] = JSON.parse(text("out/golden/vectors.json"));
...
async function deployAndInstall() {
  ...
  const splitter = await viem.deployContract("RoyaltySplitter", [owner.account.address, GALLERY]);
  const c = await viem.deployContract("ExecutedPoetry", [rt.address, splitter.address, 1000n]);
  await c.write.setPoems([manifest.map((m) => m.title), manifest.map((m) => m.poem)]);
  return { conn, viem, rt, c, splitter, owner, other };
}

const b64ToUtf8 = (dataUri: string) =>
  Buffer.from(dataUri.slice(dataUri.indexOf(",") + 1), "base64").toString("utf8");

describe("byte-for-byte assembly (unrecorded score)", () => {
  let c: any;
  before(async () => { ({ c } = await deployAndInstall()); });

  it("svg(id) matches build.py golden out/<slug>.svg", async () => {
    for (let id = 0; id < manifest.length; id++) {
      const got = await c.read.svg([BigInt(id)]);
      assert.equal(got, text(`out/${manifest[id].slug}.svg`), `svg mismatch id=${id}`);
    }
  });

  it("html(id) with count=0 matches build.py golden out/<slug>.html", async () => {
    for (let id = 0; id < manifest.length; id++) {
      const got = await c.read.html([BigInt(id)]);
      assert.equal(got, text(`out/${manifest[id].slug}.html`), `html mismatch id=${id}`);
    }
  });

  it("titles(id) / poems(id) return the poem verbatim (Etherscan Read tab)", async () => {
    for (let id = 0; id < manifest.length; id++) {
      assert.equal(await c.read.poems([BigInt(id)]), manifest[id].poem);
      assert.equal(await c.read.titles([BigInt(id)]), manifest[id].title);
    }
  });
});
...
describe("record() + recorded svg (byte-for-byte)", () => {
  ...
  before(async () => { ({ c, other } = await deployAndInstall()); });
  ...
  it("permissionless record then svg(id) matches golden (5-row, 6-row wrap, formatting)", async () => {
    for (const v of vectors) {
      ...
      const got = await c.read.svg([BigInt(v.id)]);
      assert.equal(got, text(`out/golden/${v.slug}-recorded.svg`), `recorded svg mismatch id=${v.id}`);
      assert.equal(await c.read.counts([BigInt(v.id)]), 1n, `count id=${v.id}`);
    }
  });

  it("html(id) injects the current count (let n=<count>)", async () => {
    const id = vectors[0].id;
    const html = await c.read.html([BigInt(id)]);
...
describe("tokenURI", () => {
  ...
  it("returns on-chain JSON with title, statement, image=svg, animation_url=html", async () => {
    ...
    assert.equal(json.name, manifest[id].title);
    ...
    assert.equal(traits.Title, manifest[id].title);
...
describe("ERC721 + ERC2981 royalties", () => {
  ...
  before(async () => { ({ c, splitter, owner } = await deployAndInstall()); });
  ...
  it("mint issues the exhibited 7 (ids 0..6); the other 7 stay on-chain, unminted", async () => {
    ...
    // id 7 (unselected): no token, but the poem is on-chain and still renders
...
describe("record guards + owner mutability", () => {
  ...
  before(async () => { ({ c, owner } = await deployAndInstall()); });
  ...
  it("record reverts when env lacks the javascript: prefix or is too long", async () => {
    ...
    const long = "javascript:" + "x".repeat(40); // > 44 bytes
